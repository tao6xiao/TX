...
public class IssueHandler {
    ...
    public String handIssuesByIds(int siteId, @RequestParam List<Integer> ids) throws RemoteException, BizException {
        String logDesc = "处理问题" + LogUtil.paramsToLogString(Constants.DB_FIELD_SITE_ID, siteId, Constants.IDS, ids);
        return LogUtil.controlleFunctionWrapper(() -> {
            issueService.handIssuesByIds(siteId, ids);
            return null;
    ...
    public String ignoreIssuesByIds(int siteId, @RequestParam List<Integer> ids) throws RemoteException, BizException {
        String logDesc = "忽略问题" + LogUtil.paramsToLogString(Constants.DB_FIELD_SITE_ID, siteId, Constants.IDS, ids);
        return LogUtil.controlleFunctionWrapper(() -> {
            issueService.ignoreIssuesByIds(siteId, ids);
            return null;
        }, OperationType.UPDATE, LogUtil.buildFailOperationLogDesc(logDesc), LogUtil.getSiteNameForLog(siteApiService, siteId));
    }
    ...
    public String delIssueByIds(int siteId, @RequestParam List<Integer> ids) throws RemoteException, BizException {
        String logDesc = "删除问题" + LogUtil.paramsToLogString(Constants.DB_FIELD_SITE_ID, siteId, Constants.IDS, ids);
        return LogUtil.controlleFunctionWrapper(() -> {
            issueService.delIssueByIds(siteId, ids);
            return null;
    ...
    public String updateDeptByIds(int siteId, @RequestParam List<Integer> ids, int deptId) throws RemoteException, BizException {
        String logDesc = "修改所属部门" + LogUtil.paramsToLogString(Constants.DB_FIELD_SITE_ID, siteId, Constants.IDS, ids);
        return LogUtil.controlleFunctionWrapper(() -> {
            issueService.updateDeptByIds(siteId, ids, deptId);
            return null;
        }, OperationType.UPDATE, LogUtil.buildFailOperationLogDesc(logDesc), LogUtil.getSiteNameForLog(siteApiService, siteId));
    }
