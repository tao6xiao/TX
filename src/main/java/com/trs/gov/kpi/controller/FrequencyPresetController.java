...
public class FrequencyPresetController {
    ...
    public Object addFrequencyPreset(@ModelAttribute FrequencyPresetRequest request) throws BizException, RemoteException {
        if (!authorityService.hasRight(request.getSiteId(), null, Authority.KPIWEB_INDEXSETUP_ADDPREFREQ) && !authorityService.hasRight(null, null, Authority.KPIWEB_INDEXSETUP_ADDPREFREQ)) {
            throw new BizException(Authority.NO_AUTHORITY);
        }
    ...
    public Object updateFrequencyPresetBySiteIdAndId(@ModelAttribute FrequencyPreset preset) throws BizException, RemoteException {
        if (!authorityService.hasRight(preset.getSiteId(), null, Authority.KPIWEB_INDEXSETUP_UPDATEPREFREQ) && !authorityService.hasRight(null, null, Authority
                .KPIWEB_INDEXSETUP_UPDATEPREFREQ)) {
            throw new BizException(Authority.NO_AUTHORITY);
        }
    ...
    public Object deleteFrequencyPreset(@RequestParam("siteId") Integer siteId, @RequestParam("id") Integer id) throws BizException, RemoteException {
        if (!authorityService.hasRight(siteId, null, Authority.KPIWEB_INDEXSETUP_DELETEPREFREQ) && !authorityService.hasRight(null, null, Authority.KPIWEB_INDEXSETUP_DELETEPREFREQ)) {
            throw new BizException(Authority.NO_AUTHORITY);
        }
